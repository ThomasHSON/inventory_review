...
type SortField = 'CODE' | 'NAME' | 'note';
type SortDirection = 'asc' | 'desc';

const AliasConfigModal: React.FC<AliasConfigModalProps> = ({ isOpen, onClose, selectedRecord }) => {
  ...
  const loadAliases = async () => {
    ...
    try {
      setLoading(true);
      const data = await ApiService.getAliases(selectedRecord);
  ...
  const handleNoteChange = (code: string, newNote: string) => {
    setItems(items.map(item => 
      item.CODE === code ? { ...item, note: newNote } : item
    ));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await ApiService.updateAliases(selectedRecord, items);
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      if (file) {
        try {
          setLoading(true);
          await ApiService.importAliasesExcel(selectedRecord, file);
          await loadAliases(); // Reload data after import
        } catch (error) {
  ...
  const handleExport = async () => {
    ...
    try {
      await ApiService.exportAliases(selectedRecord, items);
    } catch (error) {
      ...
      <div className="bg-white rounded-lg w-full max-w-6xl max-h-[90vh] overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-semibold text-gray-900">{t('aliasSettings')}</h2>
        ...
        <div className="p-6">
          ...
          {!selectedRecord ? (
            <div className="text-center py-8 text-gray-500">
              <p>Please select a merged record first to view alias data.</p>
            </div>
          ) : (
            <div className="overflow-x-auto max-h-96">
              {loading ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  <p>No alias data found for this record.</p>
                </div>
              ) : (
                <table className="w-full">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <SortableHeader field="note">{t('note')}</SortableHeader>
                    </tr>
                  </thead>
...
export default AliasConfigModal;